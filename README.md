...
1. **Если есть git**
   ...
   - Вставьте строку
     ```
     git clone https://github.com/ValekusVachpekus/AKH-MVD
     ```
   - Готово!
...
1. **Редактирование файла**
   - Откройте файл `!AHK_MVD_test.ahk` в блокноте или любом текстовом редакторе (VS Code, Notepad++)
   - Укажите данные:
     ```ahk
     way → [Путь до папки игры]
     CRMP_USER_NICKNAME → [Ваш никнейм]
     poz → [Ваш позывной/Фамилия]
     dolz → [Ваша должность]
     zvan → [Ваше звание]
     goska → [Ваша организация]
     ```
...
2. **Права администратора**
   - Щелкните ПКМ по файлу `!AHK_MVD_test.ahk`
